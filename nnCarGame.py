...
class Car:
  def __init__(self, sizes):
    ...
    self.biases = [np.random.randn(y, 1) for y in sizes[1:]] #Bias to be added to layer[i] is biases[i-1]
    self.weights = [np.random.randn(y, x) for x, y in zip(sizes[:-1], sizes[1:])] #Weight for layer[i] is weights[i]
